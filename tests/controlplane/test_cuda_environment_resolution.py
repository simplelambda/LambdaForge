...
class UnsupportedPythonDoctorTransport(DoctorTransport):
    """Expose an otherwise reachable environment whose Python is below the framework floor."""

    def run(self, command: Sequence[str], *, cwd: str | Path | None = None) -> CommandResult:
        if command and command[-1] == "--version":
            return CommandResult(0, "Python 3.9.21\n")
...
def test_managed_resolution_rejects_python_below_framework_floor_before_gpu_work() -> None:
    """Report the real configuration fix instead of failing later while installing the wheel."""
    transport = ResolutionTransport(
        driver="575.51.03",
        capabilities=("9.0",),
        available={"cu128": "2.8.0+cu128"},
        python="3.9",
    )
    profile = ClusterProfile(
        "h100",
    ...
    with pytest.raises(RuntimeError, match=r"requires Python >= 3\.10.*clusters\.h100\.python"):
        CudaCompatibilityResolver().resolve(profile, transport)

    assert not any(command and command[0] == "nvidia-smi" for command in transport.commands)
...
def test_doctor_warns_that_legacy_python_disables_managed_fallback() -> None:
    ...
    report = Doctor(
        ClusterCatalog({"legacy-python": profile}),
