"""Immutable v1/v2 manifest for published logical datasets."""
...
from lambdaforge.ImmutableJson import FrozenJsonMapping, JsonResult
...
class DatasetArtifact(JsonResult):
    ...
    def __init__(
        ...
        *,
        ...
        artifacts: Sequence[StoredArtifact | Mapping[str, Any]],
        created_at_utc: str,
        ...
        normalized_partitions = {
            str(key): {str(item): int(count) for item, count in values.items()}
            for key, values in (partitions or {}).items()
        }
        if any(value < 0 for value in normalized_splits.values()) or any(
            value < 0 for values in normalized_partitions.values() for value in values.values()
        ):
        ...
        self.artifacts = tuple(
            value if isinstance(value, StoredArtifact) else StoredArtifact.from_mapping(value)
            for value in artifacts
        )
        self.index = FrozenJsonMapping(index or {})
        self.target_schema = FrozenJsonMapping(target_schema or {})
        self.global_assets = {
            str(key): value if isinstance(value, DatasetAsset) else DatasetAsset.from_mapping(value)
    ...
    def create_v2(
        ...
        *,
        ...
        content_id = index.identity(
            {key: value.identity_dict() for key, value in sorted(globals_value.items())}
        )
        identity = build_provenance.get("identity", build_provenance)
        if not isinstance(identity, Mapping):
            raise TypeError("Dataset build provenance identity must be a mapping.")
        build_id = cls._digest({"build_identity_version": 1, "provenance": dict(identity)})
    ...
    def read_json(cls, path: str | Path) -> DatasetArtifact:
        ...
        return cls(
            ...
            build_id=str(value["build_id"]) if value.get("build_id") else None,
            name=str(value["name"]),
