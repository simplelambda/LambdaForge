"""Reusable Lightning module for ordinary Python training code."""
...
class LightningModule(LightningModuleBase):
    ...
    optimizer_group_kwargs : Mapping[str, Mapping[str, Any]] | None
        ...
        ``model.parameter_groups()``. Remaining trainable Lightning-module parameters form
        a ``module`` group.
    ...
    model_input_keys : Sequence[str] | Mapping[str, str] | None
        ...
        inside a Work without another training abstraction.
    model_output_key : str
        Key used when the model returns a raw tensor.
    logging : TrainingLoggingConfig | Mapping[str, Any] | None
    ...
    def __init__(
        ...
        logging: TrainingLoggingConfig | Mapping[str, Any] | None = None,
    ) -> None:
        super().__init__()
        ...
        self.logging = TrainingLoggingConfig.from_value(logging)
    ...
    def _optimizer_parameters(self) -> Any:
        ...
        for raw_name, raw_parameters in provided.items():
            name = str(raw_name)
            if not name.strip() or name == "module":
                raise ValueError("Model parameter group names must be non-empty and not 'module'.")
        ...
        if remaining:
            module_group: dict[str, Any] = {"params": remaining}
            module_group.update(self.optimizer_group_kwargs.get("module", {}))
            groups.append(module_group)
    ...
    def compute_loss_breakdown(
        ...
        if total is None:
            raise RuntimeError("LightningModule requires at least one loss.")
    ...
    def detach_tree(value: Any) -> Any:
        ...
        if isinstance(value, dict):
            return {key: LightningModule.detach_tree(item) for key, item in value.items()}

        if isinstance(value, list):
            return [LightningModule.detach_tree(item) for item in value]

        if isinstance(value, tuple):
            return tuple(LightningModule.detach_tree(item) for item in value)
