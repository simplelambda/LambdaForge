"""Configurable long short-term memory sequence model."""
...
class LSTMModel(Model):
    """Batch-first LSTM with optional recurrent projection and output head."""
    ...
    def __init__(
        ...
        if projection_size < 0 or projection_size >= hidden_size:
            raise ValueError("projection_size must be in [0, hidden_size).")
        ...
        recurrent_features = projection_size if projection_size > 0 else hidden_size
        encoded_features = recurrent_features * (2 if bidirectional else 1)
        self.output = (
