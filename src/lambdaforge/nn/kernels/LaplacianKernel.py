"""Laplacian kernel over L1 distances."""
...
class LaplacianKernel(Kernel):
    r"""Compute ``exp(-||x-y||_1 / length_scale)`` pairwise."""
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        if x.device != self.raw_length_scale.device or x.dtype != self.raw_length_scale.dtype:
            raise ValueError("Inputs and LaplacianKernel scale must share device and dtype.")
        return torch.exp(-torch.cdist(x, y, p=1.0) / self.length_scale)
