"""Implementation of the MinPooling object."""
...
class MinPooling(Pooling):
    r"""Min pooling over the set dimension.

    Selects the minimum value along each feature dimension across the set
    of valid elements:

    .. math::

        z_d = \min_{i \in \mathcal{V}} h_{i,d}

    Invalid positions (mask = 0) are filled with :math:`+\infty` before the
    min, so they never win the competition.  When no mask is supplied, all
